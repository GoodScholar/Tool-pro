...
export default {
  ...
  methods: {
    ...
    modelling() {
      ...
      const img = textureLoader.load(require('../../public/img/home3.jpeg'))

      const geometry = new THREE.SphereGeometry(130, 256, 256) // 球体网格模型
      const material = new THREE.MeshLambertMaterial({
        map: img, // 设置颜色贴图属性值
        side: THREE.DoubleSide // 双面渲染
      })
      const meshSphere = new THREE.Mesh(geometry, material) // 网格模型对象Mesh
      meshSphere.name = '球体容器'
      this.mygroup.add(meshSphere)

      const canvasText = this.getcanvers('进门') // 生成一个canvers 文字图案对象
      const texture = new THREE.CanvasTexture(canvasText)
      const geometryText = new THREE.PlaneGeometry(16, 10, 60, 60)
    ...
    addAnimation() {
      ...
      const times = [0, 3600] // 创建帧动画序列
      // eslint-disable-next-line camelcase
      const position_x = [0, 360] // 离散属性值
      const keyframe = new THREE.KeyframeTrack(
        'meshSphere.rotation[y]',
        times,
        position_x
      )
      const duration = 100 // 持续时间
      const cilp = new THREE.AnimationClip('sphereRotate', duration, [keyframe]) // 剪辑 keyframe对象
    ...
    sceneInit() {
      ...
      const ambient = new THREE.AmbientLight(0x444444, 3) // 添加光源  颜色和光照强度
    ...
    getcanvers(text) {
      ...
      const c = canvasText.getContext('2d')
      // 矩形区域填充背景
      ...
      const texture = new THREE.CanvasTexture(canvasText) // Canvas纹理
      const geometryText = new THREE.PlaneGeometry(16, 10, 60, 60) // 生成一个矩形平面
      const materialText = new THREE.MeshPhongMaterial({
        map: texture, // 设置纹理贴图
        side: THREE.DoubleSide // 双面渲染
      })
    ...
    changeScene(type) {
      ...
      for (const item of this.scene.children[2].children) {
        if (item.name === '球体容器') {
          // 切换贴图 进入下一张贴图
          const material = new THREE.MeshLambertMaterial({
            map: img, // 设置颜色贴图属性值
    ...
    getIntersects(event) {
      ...
      const raycaster = new THREE.Raycaster() // 生成射线
      const mouse = new THREE.Vector2()
      const container = this.$refs.threeDom
