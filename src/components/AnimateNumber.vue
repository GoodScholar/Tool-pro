...
export default {
  ...
  mounted() {
    ...
    if (this.mode === MANUAL) {
      ...
      if (this._fromColor && this._toColor) {
        const { r, g, b } = hexToRGB(this._fromColor)
        this.numColor = {
  ...
  methods: {
    updateNumber(state) {
      ...
      this.numColor = {
        color: `rgb(${parseInt(state.r)}, ${parseInt(state.g)}, ${parseInt(
          state.b
        )})`
      }
    },
    makeOptions() {
      ...
      const to =
        typeof this._to === 'string'
          ...
          : { x: this._to }
      if (this._fromColor && this._toColor) {
        const fromRGB = hexToRGB(this._fromColor)
        extend(from, fromRGB)
        const toRGB = hexToRGB(this._toColor)
        extend(to, toRGB)
      }
      const duration =
    ...
    reset(from, to, fromColor, toColor) {
      ...
      this.options.to =
        typeof to === 'string' ? { x: parseFloat(to) } : { x: to }
