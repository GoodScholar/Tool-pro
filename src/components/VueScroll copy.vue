...
<script>
import '@/utils/scroll/animationFrame'
// import arrayEqual from '@/utils/scroll/arrayEqual'
import copyObj from '@/utils/scroll/copyObj'
...
export default {
  ...
  computed: {
    ...
    defaultOption() {
      return {
        ...
        singleHeight: 0, // 单步运动停止的高度(默认值0是无缝不停止的滚动) direction => 0/1 单条数据宽度有值 hoverStop关闭
        singleWidth: 0, // 单条数据宽度有值 hoverStop关闭
        ...
        // todo 滑动tab栏
        switchSingleStep: 134, // 左右点击滑动时的步长
        switchDelay: 400, // 点击滑动时的延迟时间
        // todo 滑动tab栏
    ...
    _move() {
      ...
      this.reqFrame = requestAnimationFrame(
        function () {
          ...
          if (this.singleWaitTimer) clearTimeout(this.singleWaitTimer)
  ...
  beforeCreate() {
    ...
    this.singleWaitTimer = null // single 单步滚动的定时器
    this.isHover = false // mouseenter mouseleave 控制this._move()的开关
    this.ease = 'ease-in'
  },
  beforeDestroy() {
    this._cancle()
    clearTimeout(this.singleWaitTimer)
  }
}
</script>
