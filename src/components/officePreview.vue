      ...
      <div>
        ...
        <div v-if="fileType === 1">
          <div v-if="pdfData.pageCount > 1">
            ...
            ></el-button>
            <span style="font-size: 14px; font-weight: 550; margin: 0 10px">
              {{ pdfData.currentPage }} / {{ pdfData.pageCount }}
            </span>
            <el-button
              type="primary"
              size="mini"
          ...
          <div
            :style="{
              height: fullscreen
                ? clientHeight - 200 + 'px'
                : (clientHeight - 400 > 800 ? 800 : clientHeight - 400) + 'px'
            }"
        ...
        <div
          v-if="!fileType"
          style="line-height: 50px; text-align: center; font-size: 16px"
        >
          文件格式不支持，仅支持pdf,docx,xlsx,xls文件预览
        </div>
      </div>
      <div style="text-align: right; margin-top: 5px">
        <el-button size="small" @click="handleClose" class="raise">
          关闭
        </el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    getFileType() {
      ...
      const fileName = this.file.name.lastIndexOf('.') // 取到文件名开始到最后一个点的长度
      const fileNameLength = this.file.name.length // 取到文件名长度
      const fileFormat = this.file.name.substring(fileName + 1, fileNameLength) // 截
    ...
    initDocPreview() {
      ...
      try {
        ...
        this.docHtml =
          '<h1 style="text-align: center">' + this.emptyTips + '</h1>'
      }
    },
    /**
     * 表格数据预览
    ...
    getSheetNameTable(sheetName) {
      try {
        ...
        this.excel.SheetActiveTable =
          '<h4 style="text-align: center">' + this.emptyTips + '</h4>'
      }
    }
  },
  created() {
